...
\begin{map}[H]
	...
	\caption{The bathymetric raster map of the seafloor of the Pacific Ocean}\label{map:PO}
\end{map}

\begin{map}[H]
	\centering
	\includegraphics[width=14cm]{Fig-GMT-GeolW.jpg}
	\caption{The geologic main structure of the Pacific Ocean}\label{map:GeolW}
\end{map}

\begin{map}[H]
	\centering
	\includegraphics[width=14cm]{Fig-GMT-TectSeaf.jpg}
	\caption{The tectonic structure and seafloor main lineaments on the Pacific Ocean}\label{map:TectW}
\end{map}
...
\begin{map}[H]
	...
	\caption{Study area: bathymetric map of the Sea of Okhotsk and \ac{KKT}}\label{map:KKT1}
\end{map}
...
\begin{map}[H]
	...
	\caption{Study region: Sea of Okhotsk and Kamchatka area. Mercator oblique projection}\label{map:KKT9}
\end{map}
...
The tectonic properties of the study area mutually affect both land and submarine geomorphology. Thus, as noted by \cite{Pflanz}, the location of the  \ac{KKT} at the meeting place of active Kuril-Kamchatka \index[places]{Kuril-Kamchatka Arc} and Aleutian arcs \index[places]{Aleutian Arc} causes the coastline of the Kamchatka Peninsula \index[places]{Kamchatka Peninsula} to be affected by strong tectonic activities. Fracture zones have variable influence on the uplift of the Kamchatka Peninsula. The geomorphic consequence include raise of the multi- level, highly uplifted marine terraces displaced along the active tectonic faults. Furthermore, the tectonic movements cause the uplift of the coastal sediments \cite{Pflanz}. The \ac{KKT} belongs to the Chilean tectonic subduction type \index[places]{Chile}, which has a gentle subduction dip advancing at a speed of ca. 2 cm yr–1 and inter-seismic elastic shortening of 1–2 mm yr−1 \cite{DeMetsetal2010}. Strong lateral velocity in the shallower mantle wedge accounts for both the compressional subduction tectonics and back arc compression in the \ac{KKT}. This results in the seismic-velocity anomalies in the mantle under the \ac{KKT}, as shown on the tomographic images by Yoshida (2017). Despite the age of the Pacific Plate, there is no sharp variation of the age on the seafloor along the \ac{KKT} \cite{RenkinSclater}, and the old Pacific Plate \index[places]{Pacific Plate} is rejuvenated at the north of the \ac{KKT} around the Meiji Guyot seamount \index[places]{Meiji Guyot seamount}.
 ...
 \begin{map}[H]
	...
	\caption{Composite overlay of the 3D-topographical mesh model.}\label{map:KKT11}
\end{map}
...
\begin{map}[H]
	...
	\caption{Model of color geoid image of the \ac{KKT}}\label{map:KKT12}
\end{map}

 \begin{map}[H]
	\centering
		\includegraphics[width=10cm]{Fig-KKT13.jpg}
	\caption{Model of the marine free-air gravity anomaly of the \ac{KKT}}\label{map:KKT13}
\end{map}
...
\begin{map}[H]
	...
	\caption{Topographic surface modelling, \ac{KKT}}\label{map:KKT14}
\end{map}
...
\begin{map}[H]
	...
	\caption{Gravity surface modelling, \ac{KKT}}\label{map:KKT15}
\end{map}
...
\begin{map}[H]
	...
	\caption{Geological settings of the \ac{KKT} study area}\label{map:KKT2}
\end{map}
...
\begin{figure}[H]
	...
	\caption{3D histograms: depths distribution in the southern part of the Sea of Okhotsk and \ac{KKT}}\label{fig:KKT3}
\end{figure}
...
\begin{map}[H]
	...
	\caption{Bathymetric and general map of the Aleutian Trench area from the ETOPO1 grid}\label{map:A1}
\end{map}
...
\begin{map}[H]
	...
	\caption{Contour map showing area of the Aleutian Trench}\label{map:A2}
\end{map}
...
\begin{map}[H]
	...
	\caption{Modelling 3D mesh grid of the Aleutian Islands and Aleutian Trench area}\label{map:A3}
\end{map}
...
\begin{map}[H]
	...
	\caption{Geoid map of the Aleutian Islands and Aleutian Trench area}\label{map:A4}
\end{map}

\begin{map}[H]
	\centering
		\includegraphics[width=10cm]{Fig-A5.jpg}
	\caption{Gravimetry map of the Aleutian Islands and Aleutian Trench area}\label{map:A5}
\end{map}
...
\begin{map}[H]
	...
	\caption{Bathymetry of the \ac{MAT}}\label{map:MAT1}
\end{map}
...
\begin{map}[H]
	...
	\caption{Topographic contour map of the study area: \ac{MAT}}\label{map:MAT2}
\end{map}
...
\begin{map}[H]
	...
	\caption{3D topographic mesh map plot on the bathymetry of the \ac{MAT}}\label{map:MAT3}
\end{map}
...
\begin{map}[H]
	...
	\caption{Geoid model of the \ac{MAT}}\label{map:MAT4}
\end{map}
...
\begin{map}[H]
	...
	\caption{Geological settings of the \ac{MAT}}\label{map:MAT5}
\end{map}
...
\begin{map}[H]
	...
	\caption{Gravity model of the \ac{MAT}}\label{map:MAT6}
\end{map}
...
\begin{map}[H]
	...
	\caption{Vertical gravity model of the \ac{MAT}}\label{map:MAT7}
\end{map}
...
\begin{map}[H]
	...
	\caption{Surface modelling: topographic map generated from the ASCII xyz data, \ac{MAT}}\label{map:MAT8}
\end{map}
...
\begin{map}[H]
	...
	\caption{Surface modelling: gravity model generated from the ASCII xyz data, \ac{MAT}}\label{map:MAT9}
\end{map}
...
\begin{map}[H]
	...
	\caption{Bathymetry of the \ac{PCT}, cartographic base: ETOPO1}\label{map:PCT1}
\end{map}
...
\begin{map}[H]
	...
	\caption{Geological map, Peru-Chile Trench}\label{map:PCT2}
\end{map}
...
\begin{map}[H]
	...
	\caption{Topographic surface modelling, Peru-Chile Trench}\label{map:PCT3}
\end{map}
...
\begin{map}[H]
	...
	\caption{Gravity surface modelling, Peru-Chile Trench}\label{map:PCT4}
\end{map}
...
\begin{map}[H]
	...
	\caption{Bathymetric and general map of the Tonga and Kermadec trenches}\label{map:TK1}
\end{map}
...
\begin{map}[H]
	...
	\caption{Topographic map of the New Britain and San Cristobal trenches}\label{map:TK5}
\end{map}

\begin{map}[H]
	\centering
		\includegraphics[width=13cm]{Fig-NBT3.jpg}
	\caption{Geoid model of the New Britain and San Cristobal trenches}\label{map:TK5}
\end{map}
...
\begin{map}[H]
	...
	\caption{Geological and tectonic map of the New Britain and San Cristobal trenches}\label{map:TK5}
\end{map}
...
\begin{map}[H]
	...
	\caption{Gravimetry model of the New Britain and San Cristobal trenches}\label{map:TK5}
\end{map}
...
\begin{map}[H]
	...
	\caption{Study area and digitized cross-section profiles of the Philippine Trench area. QGIS}\label{map:GML1}
\end{map}
...
\begin{figure}[H]
	...
	\caption{Ridgeline plot on the topographic profiles, Philippine Trench area. Plotting: R.}\label{fig:GML4}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Regression analysis: profiles of the Philippine Trench area. Plotting: R code \ref{R:05}}\label{fig:GML5}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Letter-value plot on data distribution of the topographic profiles. Plotting: Python.}\label{fig:GML6}
\end{figure}
...
\begin{map}[H]
	...
	\caption{Bathymetric map of the study area based on the SRTM high-resolution 15 sec grid}\label{map:M1}
\end{map}
...
\begin{map}[H]
	...
	\caption{Geologic map of the Manila Trench based on the ETOPO1 1 min grid.}\label{map:M2}
\end{map}
...
\begin{map}[H]
	...
	\caption{Model of color geoid image (left) and gravimetry (right) of the Manila Trench}\label{map:M3}
\end{map}
...
\begin{map}[H]
	...
	\caption{Model of color geoid image (left) and gravimetry (right) of the Manila Trench}\label{map:M4}
\end{map}
...
\begin{map}[H]
	...
	\caption{3D mesh map of the Manila Trench geomorphology}\label{map:M5}
\end{map}
...
\begin{map}[H]
	...
	\caption{Bathymetric map of the East China Sea, northern Philippine Sea, Ryukyu Arc and Trench}\label{map:R1}
\end{map}
...
\begin{map}[H]
	...
	\caption{Geologic map of the Ryukyu Trench based on ETOPO1, GMT}\label{map:R2}
\end{map}
...
\begin{map}[H]
	...
	\caption{Map of the geoid: Ryukyu Trench area}\label{map:R3}
\end{map}

\begin{map}[H]
	\centering
		\includegraphics[width=10cm]{Fig-R4.jpg}
	\caption{Map of the gravimetry: Ryukyu Trench area}\label{map:R4}
\end{map}
...
\begin{map}[H]
	...
	\caption{3D mesh geomorphological modelling of the Ryukyu Trench}\label{map:R5}
\end{map}
...
\begin{map}[H]
	...
	\caption{Geological map of the Yap and Palau trenches}\label{map:YPT2}
\end{map}

\begin{map}[H]
	\centering
		\includegraphics[width=13cm]{Fig-YPT3.jpg}
	\caption{Geophysical gravity map of the Yap and Palau trenches.}\label{map:YPT3}
\end{map}
...
\begin{map}[H]
	...
	\caption{Topographic map of the Yap and Palau trenches.}\label{map:YPT1}
\end{map}

\begin{map}[H]
	\centering
		\includegraphics[width=13cm]{Fig-YPT4.jpg}
	\caption{Geoid model of the Yap and Palau trenches.}\label{map:YPT4}
\end{map}
...
\begin{map}[H]
	...
	\caption{Mariana Trench. Mapping: GMT}\label{map:BathyMT}
\end{map}
...
\begin{map}[H]
	...
	\caption{Map of the digitized profiles, Mariana Trench (left). Coordinates for the selected profile (Nr. 21), Table (right): (1) fragment in geographic coordinate system EPSG: 4326 WGS 84. (2), right: fragment in Cartesian coordinate system, re-projected}\label{map:GP-2}
\end{map}
...
\begin{figure}[H]
	...
	\caption{Box plots on data distribution, Mariana Trench profiles. \\ R code: \ref{R:02}}\label{fig:4-3}
\end{figure}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=13cm]{Fig-4-3-1b.jpg}\caption{Violin plots: depth distribution for 25 profiles of the Mariana Trench. R code: \ref{R:25}}\label{fig:4-3-1b}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Kernel density estimation for 25 bathymetric profiles, by R code: \ref{R:19}}\label{fig:4-3-1k}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Compositional chart of the geometry of the Mariana Trench, R}\label{fig:4-3-W}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Classification of k-means clustering by plates (Mariana Trench), R code \ref{R:09}}\label{fig:4-3-14}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Normalized steepness slope angle, profiles of the Mariana Trench, by R.}\label{fig:4-4}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Pearson and Spearman correlation matrices for the Mariana Trench, by R code \ref{R:41}}\label{fig:4-56}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Hierarchical dendrogram tree of the profiles, Mariana Trench, by R code: \ref{R:06}}\label{fig:4-3-8ab}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Hierarchical clustering with p-values, multiscale bootstrap probability, by R code: \ref{R:06}}\label{fig:4-3-8de}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{General factor saturation ($\omega$h) and 3 factors $\omega$ coefficient, by R code \ref{R:26}}\label{fig:4-3-9gh}
\end{figure}
...
\begin{figure}[H]
	\centering
		...
		\subfloat {\includegraphics[width=7cm]{Fig-4-3-10b.jpg}}
			\hspace{3mm}
	\caption{Euler-Venn diagram. Left: tectonic plates. Right: impact factors, by R code \ref{R:23}}\label{fig:4-3-10ab}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Python-based graph (Matplotlib and Seaborn): \ac{KDE} of the bathymetric profiles.}\label{fig:AR-3}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Violin plots showing depth ranges across the Mariana Trench. Python, by code \ref{P:01}}\label{fig:COMU-3}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Euler-Venn diagrams, Mariana Trench project. Plotting: Python.}\label{fig:GP-9}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Area chart and facet grid of the profiles, Mariana Trench. Plotting: Python (Seaborn).}\label{fig:MedFAR-3}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Regression analysis: facet grid of the profiles, Mariana Trench. Plotting: Python (Seaborn)}\label{fig:MedFAR-4}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Letter-value plot for data distribution by profiles, Mariana Trench. Python (Seaborn).}\label{fig:MedFAR-5}
\end{figure}
...
\begin{figure}[H]
	\centering
		...
		\subfloat {\includegraphics[width=7cm]{Fig-COMU-9.jpg}}
			\hspace{3mm}
	\caption{Correlation matrices for the variables of Mariana Trench: Pearson (left), Kendall (right)}\label{fig:COMU-9}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Hierarchical cluster dendrogram of the profiles crossing Mariana Trench. Python}\label{fig:EgeJFAS-5}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{One-way \ac{ANOVA} box plots , Mariana Trench.}\label{fig:EgeJFAS-10}
\end{figure}
...
The reason to choose the radar charts is that there are various statistical values that camn be visualized by profiles, which requires a certain visualization technique for the facetted multi-plots (Figure \ref{fig:AR-5}). The series of the radar charts were plotted by libraries Math, Pandas, NumPy, Matplotlib, Seaborn and OS. The algorithm for the radar charts was taken from the Matplotlib library and the Python code is provided below: \ref{P:18}. 
...
\begin{figure}[H]
	...
	\caption{Stacked bar plots: variations of depths by profiles, Mariana Trench. Python.}\label{fig:AR-6}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Stacked bar charts, Mariana Trench geological factors. Python.}\label{fig:AR-7}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Pie charts of the bathymetric samples distribution by tectonic plates. Python.}\label{fig:AR-8}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Data distribution analysis: maximal depths by profiles, Mariana Trench. Python.}\label{fig:GP-3}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Data distribution analysis: maximal depths by profiles, Mariana Trench. Python (Bokeh)}\label{fig:GP-5}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Isotonic regression for the geological and bathymetry variables, Mariana Trench. Python}\label{fig:GP-7}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Median stacked profiles of the \ac{KKT}, southern (A) and northern (B) segments.}\label{fig:KKT4}
\end{figure}
...
\begin{map}[H]
	...
	\caption{Enlarged view of the profiles: numbering starts from south to north for both segments.}\label{map:KKT5}
\end{map}
...
\begin{figure}[H]
	...
	\caption{Northern part of the \ac{KKT}: a graph based on the table reshaped by AWK.}\label{fig:KKT6}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Southern part of the \ac{KKT}: a graph based on the table reshaped by AWK script.}\label{fig:KKT7}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{2D histograms and rose diagrams of depths distribution, \ac{KKT}}\label{fig:KKT8}
\end{figure}
...
The \ac{GMT} based modelling demonstrated following results. The topography in the study area ranges between -8200 to 2000 m (Map \ref{map:A6}). The geoid gravitational model shows the majority of values lies in the interval between -10 to 20 mGal with the lowest values along the trench. The profiles plotted in an automated regime are located with a distance of 20 km between each pair, spaced every 2 km along the profiles itself and have a total length of 400 km, that is 200 km to both sides from the trench. According to the statistical histogram modelling, the most often depth value in the study area is - 4800 with frequency 1722. 
 ...
 \begin{map}[H]
	...
	\caption{Middle America Trench: digitized cross-section profiles}\label{map:A6}
\end{map}
...
\begin{map}[H]
	...
	\caption{Topographic surface modelling from the ASCII xyz data}\label{map:A9}
\end{map}
...
\begin{map}[H]
	...
	\caption{Surface modelling from the ASCII xyz data}\label{map:A10}
\end{map}
...
\begin{map}[H]
	...
	\caption{Cross-section profiles on the selected segment of the \ac{MAT}}\label{map:MAT10}
\end{map}
...
\begin{figure}[H]
	...
	\caption{Trend regression models for the cross-section profiles, \ac{MAT}}\label{fig:MAT11}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Median stacked profiles for the Chilean (A) and Peruvian (B) parts}\label{fig:PCT6}
\end{figure}

\begin{map}[H]
	\centering
		\includegraphics[width=10cm]{Fig-PCT5.jpg}
	\caption{Digitized profiles of the \ac{PCT} for the Chilean (red) and Peruvian (yellow) segments.}\label{map:PCT5}
\end{map}

\begin{figure}[H]
	\centering
		\includegraphics[width=15cm]{Fig-PCT7.jpg}
	\caption{Histograms of the data distribution: Peruvian and Chilean segments}\label{fig:PCT7}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Histograms of the bathymetric data distribution. Left: Kermadec, right: Tonga}\label{fig:TK5}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Faceted plot of the 25 profiles crossing Philippine Trench. Plotting: \ac{QGIS}}\label{fig:GML2}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Strip plots: data distribution by tectonic plates (A) and geology (B), by Python code \ref{P:05}}\label{fig:GML7}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Distribution of the samples across tectonic plates. Plotting: Python (Matplotlib) and R.}\label{fig:GML8}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Correlation matrix (Spearman method) for visualizing impact factors. Plotting: R.}\label{fig:GML9}
\end{figure}
...
\begin{map}[H]
	...
	\caption{Two segments of the cross-section profiles digitized along the of the Manila Trench}\label{map:M6b}
\end{map}
...
\begin{map}[H]
	...
	\caption{Cross-section profiles of the Ryukyu trench: southern and northern segments}\label{map:R6}
\end{map}
...
\begin{figure}[H]
	...
	\caption{Geomorphological modelling of the profiles transecting Yap and Palau trenches.}\label{fig:YPT7}
\end{figure}

\begin{figure}[H]
	\centering
		\includegraphics[width=12cm]{Fig-YPT8.jpg}
	\caption{Statistical histograms showing bathymetric data of the Yxap and Palau trenches.}\label{fig:YPT8}
\end{figure}

\begin{figure}[H]
	\centering
		\includegraphics[width=12cm]{Fig-YPT10.jpg}
	\caption{Trend modelling showing gradient slope direction of the Palau trench}\label{fig:YPT10}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Trend modelling showing gradient slope direction of the Yap trench}\label{fig:YPT9}
\end{figure}

\begin{map}[H]
	\centering
		\includegraphics[width=12cm]{Fig-YPT6.jpg}
	\caption{Digitizing cross-section profiles of the Yap and Palau trenches.}\label{map:YPT6}
\end{map}

\begin{map}[H]
	\centering
		\includegraphics[width=13cm]{Fig-YPT5.jpg}
	\caption{3D modelling of the Yap and Palau trenches.}\label{map:YPT5}
\end{map}
...
\begin{figure}[H]
	...
	\caption{Steepness gradient of the Mariana Trench profiles: unsorted and sorted. R code \ref{R:22}}\label{fig:4-3-17}
\end{figure}
