...
\begin{acronym}[JSONP]\itemsep0pt
	...
	\acro{GSHHG}{Global Self-consistent, Hierarchical, High-resolution Geography Database}
...
\subsection{Sedimentation}
	...
	Furthermore, specifically for the Peru-Chile Trench, larger downward deflection of the plate is found along the Chiloé segment\index[places]{Peru-Chile Trench, Chiloé segment} than along the Maule segment\index[places]{Peru-Chile Trench, Maule segment} \cite{ContrerasReyesetal2013}. Hence, according to the sedimentation conditions, the Peru-Chile Trench can be divided into two main provinces: the main sediment-free province extending 8\degree S-32\degree S and the sedimentary province extending 33\degree S-57\degree S \cite{Hayes}. The margin off Isla de Chiloé\index[places]{Peru-Chile Trench, Chiloé segment} is characterized by a largely sedimented segment of the Peru-ChileTrench. This is caused by the extension of turbidite deposits seaward reaching 250 km from the trench axis \cite{Contreras2007}. The region close to the Taitao Triple Junction\index[places]{Taitao Triple Junction} is nearly devoid of sediments and has rapid narrowing of the forearc region \cite{Behrmann}. 
	\pagebreak
...
The Palau Trench \index[places]{Palau Trench} is located in the Philippine Sea\index[places]{Philippine Sea}, northwards from the Papua-New Guinea\index[places]{Papua New Guinea} and eastwards from the Philippines, west Pacific Ocean (Map. \ref{map:YPT1}).The geological settings of the Palau Trench area is shown on Map \ref{map:YPT2}. Palau Trench with the greatest depth at 8,080 m (situated in its southern part) has a straight shape of the geometric form, stretching from south-west to north-east (Map {map:YPT1} and Map {map:YPT6}), located within a square 133\degree - 136\degree E and 6\degree-8\degree N, east from the Yap Islands\index[places]{Yap Islands}. 
...
The geoid modelling of the Ryukyu Trench (Map \ref{map:R3R4}) was done using following code:  gmt grdimage geoid.egm96.grd -I+a45+nt1 -R120/134/20/33 - JD127/24/27/30/6i -Cgeoid.cpt -P -K > \$ps Example of modelling geoid of \ac{MAT} area is on Map \ref{map:MAT4}. The additional cartographic elements were added using GMT basemap module: grid, title, costline and GMT pscoast module. The full script is presented in Appendix \ref{GMT:04}.
...
The quantile regression\index[concepts]{regression, quantile } estimates conditional median\index[concepts]{statistics, median} and other quantiles\index[concepts]{statistics, quantiles} of the response geological variables. Thus the upper two rows of the plot show (Fig. 4 in \cite{Lemenkova201989}) data distribution for sediment thickness\index[concepts]{sediment thickness} (m) versus geologic parameters across tectonic plates, data distribution for the cumulative sediment thickness and slope angle degree by profiles. The quantile regressions were plotted (Fig. \ref{fig:ASE-4}) using Python code \ref{P:24} by \ac{StatsModel}. Essentially, quantile regression is another approach of the linear regression tested in the current research. The quantitative results of the quantile regression are shown on Tab. 3 in \cite{Lemenkova201989} which shows computations for the quantile regression for sediment thickness versus geologic parameters and tectonic plates. 
...
Apart from the statistical tools, a \ac{GIS} data analysis is largely used in geological sciences. Comparing to the existing approaches based on various existing \ac{GIS} software, e.g. \ac{ESRI} ArcGIS (\cite{Klauco2013}; \cite{Suetova2005a}, \cite{Lemenkova2012j}, \cite{Lemenkova2011c}), or other examples of the geospatial modelling applied for geodetic approximations \cite{Ligasetal2019}, 3D modelling (e.g. \cite{Tomaszewski}, \cite{Banasik2017}, \cite{Janecka2019}) the \ac{GMT} is notable for its scripting based methodology. Plotting a map evolves scripting and running a code from the console, unlike a traditional \ac{GIS} with existing \ac{GUI}. 

As a result of the high-resolution datasets and raster grids used in this work, we now have much better and detailed information on the bathymetry and topography of the New Zealand region and south Pacific Ocean. Therefore, using 15 arc sec resolution \ac{GEBCO} as a base map for thematic mapping is strongly recommended in future similar works. 
